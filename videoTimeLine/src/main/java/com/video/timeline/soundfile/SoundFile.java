...
public class SoundFile {
     ...
     * @return
     */
    public int[] cutTime(long startTimeMs,long endTimeMs){
        float frameDuration = (float)getSamplesPerFrame() / mSampleRate;
        int startFrameIndex = (int)(startTimeMs / (frameDuration * 1000));
        int endFrameIndex = (int)(endTimeMs / (frameDuration * 1000));
        int[] gains = new int[endFrameIndex - startFrameIndex + 1];
        for (int i = startFrameIndex; i <= endFrameIndex; i++) {
            gains[i - startFrameIndex] = mFrameGains[i];
        }
        return gains;
    }
