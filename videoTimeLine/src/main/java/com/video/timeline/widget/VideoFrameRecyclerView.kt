...
import com.video.timeline.adapter.VideoFrameAdapter
...
class VideoFrameRecyclerView @JvmOverloads constructor(
    ...
    private val videoFrameItemDecoration: FrameItemDecoration

    init {
        val videoFrameAdapter = VideoFrameAdapter(listData, frameWidth)
        videoFrameAdapter.setPreLoadNumber(10)
        adapter = videoFrameAdapter
        videoFrameItemDecoration = FrameItemDecoration(context)
        ...
        for ((index, item) in videoData.withIndex()) {
            ...
            while (widthCount < durationPx) {
                ...
                if (isFirst) { //第一帧可能不是完整帧宽度
                    left =
                        timeLineValue.time2px(((item.startAtMs % clipFrameTime) / speed).toLong())
                            .toInt()
                    itemWidth = min(
                        itemWidth,
                        frameWidth - left
